...
- `build-regressions.js`: WoW 비교 CSV 에서 perf Δ ≤ -5 인 URL 만 추출해 회귀 보고서 생성 (Job Summary 전용). `REGRESSION_THRESHOLD` 환경변수로 조정 가능

### AI 분석

- `evaluate-applied-actions.js`: `applied-actions.md` 의 적용 액션 효과를 **코드로 검증**. `history/*.json` 의 before `[적용일-7d, 적용일)` / after `[today-7d, today]` 윈도에서 host 의 device별 metric(perf/lcp/cls/tbt) 평균을 비교해 IMPROVED/DEGRADED/NEUTRAL/INSUFFICIENT_DATA 판정. content 키워드로 primary metric 추론(webp→LCP, font→CLS, JS/defer→TBT). 출력 `results/applied-eval.json` + `.md` → AI 에 fact 로 주입 + Job Summary 노출
- `generate-3m-ai-analysis.js`: 3개월 추이 AI 분석
- `generate-ai-suggestions.js`: 이번 실행 결과 기반 AI 제안 (TL;DR + Per-site Diagnosis + Top URL Actions + Cross-cutting 4섹션 구조). 레포 루트 `applied-actions.md` 의 *최근 `APPLIED_WINDOW_WEEKS` 주(기본 12주)* 분량은 **재제안 차단** 용도로 주입. **효과 검증(✅/⚠️/➖)은 더 이상 AI 추론이 아니라 `evaluate-applied-actions.js` 가 코드로 산출한 결과(`applied-eval.json`)를 fact 로 주입** — AI 는 그 결과를 다시 판정하지 않고 host 진단 컨텍스트로만 활용. Slack 에는 Top 6 URL(모바일 3 + 데스크탑 3) 액션, GitHub Job Summary 에는 전 섹션 노출
...
2. `summarize_and_notify` (위 완료 후) — 4 artifact 를 device 별 merge → 요약 / history 커밋 / 3개월 비교 / WoW 비교 / 회귀 보고서 / **적용 액션 효과 검증(evaluate-applied-actions.js)** / AI 분석 + 제안 / Slack 발송 / 노션 sync (secret 등록 시)
...
└───────────────────────────────────────────────────────────┘
                            ↓
              row detail page:
              ├ ⚠️ Regressions (perf Δ ≤ -5) bulleted list
...
- `results/regressions-wow.md`: WoW 회귀 보고서 (perf Δ ≤ -5 만 — Job Summary 전용)
...
- `results/applied-eval.json` / `.md`: 적용 액션 효과 검증 결과 (코드 산출, Job Summary 노출)
